...
intents.members = True # ملاحظة: تأكد من تفعيل خيار Server Members Intent في موقع Discord Developer Portal
...
class ControlPanelView(discord.ui.View):
    ...
    async def stats_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        guilds_text = truncate(guilds_text, 3800)
        embed = discord.Embed(title="📊 إحصائيات التواجد", description=f"إجمالي السيرفرات: **{len(bot.guilds)}**\n\n**قائمة السيرفرات:**\n{guilds_text}", color=discord.Color.blue())
    ...
    async def channel_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.guild is None:
            return await interaction.response.send_message("❌ هذا الزر يعمل فقط داخل سيرفر!", ephemeral=True)
        try:
            ...
            await interaction.response.send_message(truncate(f"❌ خطأ: {e}"), ephemeral=True)
    ...
    async def shutdown_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await bot.is_owner(interaction.user):
            return await interaction.response.send_message("❌ لا تملك صلاحية!", ephemeral=True)
        await interaction.response.send_message("👋 جاري إغلاق البوت...", ephemeral=True)
        await bot.close()
...
class DynamicCodeButton(discord.ui.View):
    ...
    async def run_dynamic_code(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        env = {'bot': bot, 'interaction': interaction, 'channel': interaction.channel, 'author': interaction.user, 'guild': interaction.guild, 'discord': discord, 'os': os, 'sys': sys}
...
async def send_panel(ctx):
    await ctx.send(embed=discord.Embed(title="🎛️ لوحة التحكم", description="...", color=discord.Color.dark_theme()), view=ControlPanelView())

@bot.command(name="برمج")
@commands.is_owner()
async def execute_code(ctx, *, code: str):
    env = {'bot': bot, 'ctx': ctx, 'channel': ctx.channel, 'author': ctx.author, 'guild': ctx.guild, 'discord': discord, 'os': os, 'sys': sys}
...
async def create_button_command(ctx, *, code: str):
    ...
    msg = await ctx.send("⚙️ **تم إنشاء زر برمجي!**", view=view)
    view.message = msg

@bot.event
async def on_command_error(ctx, error):
    if isinstance(error, commands.NotOwner):
        await ctx.send("❌ هذا الأمر لمالك البوت فقط!")

if __name__ == "__main__":
    token = os.environ.get("DISCORD_TOKEN")
    if not token:
        print("❌ لم يتم ضبط DISCORD_TOKEN.")
        sys.exit(1)
    bot.run(token)
